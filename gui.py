...
class CANStreamerGUI:
    def __init__(self, root, start_streaming_callback):
        ...
        self.address_label.grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)

        self.address_entry = tk.Entry(main_frame)
        self.address_entry.grid(row=1, column=1, padx=5, pady=5, sticky=tk.EW)

        self.port_label = tk.Label(main_frame, text="Porta UDP:")
        self.port_label.grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)

        self.port_entry = tk.Entry(main_frame)
        self.port_entry.grid(row=2, column=1, padx=5, pady=5, sticky=tk.EW)
        ...
        self.log_box.grid(row=3, column=0, columnspan=3, padx=5, pady=10, sticky=tk.EW)
        ...
        self.start_button.grid(row=4, column=1, padx=5, pady=10, sticky=tk.E)
    ...
    def start_streaming(self):
        ...
        if not selected_interface or not address or not port:
            self.log_message("Errore: Tutti i campi devono essere compilati.")
            return

        self.log_message(f"Avvio dello streaming su interfaccia {selected_interface}, indirizzo {address}, porta {port}")
        try:
            # Chiamata alla callback passata come argomento
            self.start_streaming_callback(selected_interface, address, port)
...
# Funzione di callback simulata per lo streaming (da sostituire con la tua implementazione)
def start_streaming_callback(interface, address, port):
    print(f"Streaming avviato con interfaccia {interface}, indirizzo {address}, porta {port}")


# Creazione e avvio della GUI
